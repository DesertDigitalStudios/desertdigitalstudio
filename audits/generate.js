...
const { generateAuditFiles } = require('./builder');
...
async function main() {
  ...
  if (!leadPath) {
    console.error('Usage: node audits/generate.js <lead.json> [packageId] [price]');
    process.exit(1);
  }

  const lead = JSON.parse(fs.readFileSync(path.resolve(leadPath), 'utf8'));
  const result = await generateAuditFiles({ lead, packageId, price });
