...
lines.push('## Best actionable Tucson leads', '');
if (!actionable.length) {
  lines.push('- No prime or pursue Tucson leads are in the CRM right now.');
} else {
...
if (!reachable.length) {
  lines.push('- No Tucson leads with clean email yet.');
} else {
...
if (!followups.length) {
  lines.push('- No Tucson follow-ups are overdue right now.');
} else {
...
if (reachable.length) {
  lines.push('- Start with the strongest reachable Tucson leads first.');
  lines.push('- Draft outreach for the top 3 with real emails before doing manual enrichment.');
} else if (actionable.length) {
  lines.push('- Prioritize manual enrichment for the best Tucson prime/pursue leads.');
  lines.push('- Use Instagram, Facebook, phone, or walk-in follow-up where email is missing.');
} else {
  lines.push('- Re-run import checks after the next scan cycle and keep Tucson in watch mode for now.');
}
