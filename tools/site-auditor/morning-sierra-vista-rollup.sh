...
base = Path.home() / 'Desktop' / 'Audit reports' / 'nightly-sierra-vista' / datetime.now().strftime('%Y-%m-%d')
print('# Sierra Vista Morning Roll-up')
print()
...
if not reports:
    print('No report.json files found in today\'s Sierra Vista scan folder.')
    raise SystemExit
...
print()
if not all_leads:
    print('- No strong outreach-worthy leads surfaced from today\'s Sierra Vista scan.')
else:
...
print()
if all_leads:
    print('- Pull public contact emails for the top 5 Sierra Vista leads first.')
    print('- Draft outreach only for the strongest reachable businesses.')
    print('- Skip businesses that already look polished enough online.')
else:
